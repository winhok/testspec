...
- **优先级独立判断**：测试点中的优先级仅供参考，每个用例根据具体场景独立判断；同一测试点可衍生 P0 到 P3 不同级别的用例
...
- **P0（冒烟用例）**：核心功能和关键业务流程，验证系统基本可用性（**占比约 30%**）
- **P1（主要功能）**：影响主要业务流程的重要用例（占比约 40%）
- **P2（一般功能）**：影响次要业务流程的常规用例（占比约 20%）
- **P3（边缘功能）**：轻微影响或优化类的低优先级用例（占比约 10%）
...
- **用户旅程分析**：识别用户完成核心任务的最短路径，路径上的每个关键步骤都是 P0 候选
- **失败影响分析**：分析功能失败的影响范围，阻塞性强的功能为 P0
- **使用频率分析**：高频操作场景对应的用例为 P0 候选
- **业务价值评估**：直接产生收入或核心竞争力的功能为 P0

### P0 数量动态调整

- P0 占比不足时：从 P1 中提升核心路径关键步骤、高频功能、影响范围大的用例
- P0 占比过高时：降级非核心路径、低频操作、辅助功能的用例
- 质量优先于数量，不为凑数而降低 P0 用例质量
...
- P0 冒烟用例占比约 30%
- P0 覆盖核心业务流程和关键功能入口
- 各级别分布合理（P0≈30%, P1≈40%, P2≈20%, P3≈10%）
- 未为凑数而降低 P0 用例质量
